    ...
    public void addNewsHubPayload(Payload payload) {
        try {
            ...
            if (cursor.getCount() > 0) {
                ...
                values.put(NOTIFICATION_ID, payload.getTitle());
                ...
                values.put(NOTIFICATION_SOUND, payload.getTitle());
                values.put(ADDITIONAL_PARAM, payload.getAp());
                values.put(MAX_NOTIFICATION, payload.getMaxNotification());
//        values.put(TEXT_OVERLAY, payload.getCustomNotification());
    ...
    public boolean checkIdExist(String notificationID) {
        try {
            SQLiteDatabase db = this.getWritableDatabase();
    ...
    public ArrayList<Payload> fetchNewsHubData() {
        try {
            ...
            String query = "SELECT * FROM " + TABLE_NAME + " ORDER BY + CREATED_ON DESC";
